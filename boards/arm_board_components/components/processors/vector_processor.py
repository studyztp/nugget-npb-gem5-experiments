...
class BaseVectorProcessor(BaseCPUProcessor, VectorAwareProcessor):
    def __init__(self, cores: List[VectorCore]):
        super().__init__(cores)

    @overrides(VectorAwareProcessor)
    def get_core_simobjects(self):
...
class SwitchableVectorProcessor(SwitchableProcessor, VectorAwareProcessor):
    def __init__(
        self,
        isa: ISA,
        num_cores: int,
        ...
        for warmup, evaluation in zip(
            switchable_cores["warmup_cores"],
            switchable_cores["evaluation_cores"],
        ):
            evaluation.set_cpuid(warmup.get_cpuid())
        super().__init__(
    ...
    def _get_core_switchable_cores(self, key: str):
        return self._switchable_cores[key]
    ...
    def get_core_simobjects(self):
        return [
            core.get_core_simobject()
            for core in self._get_core_switchable_cores("evaluation_cores")
        ]
